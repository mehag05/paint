...
type gctx = {
  ...
  mutable thicknessint: int;   (*created to implement thickness*)
  }
...
let top_level : gctx =
  { x = 0;
    y = 0;
    color = black;
    thicknessint = 1; (*starts with default of 1 thickness*)
     }
...
let with_thickness (g: gctx) (i: int) : gctx =
  { g with thicknessint = i }
...
let set_graphics_state (gc: gctx) : unit =
  ...
  Graphics.set_line_width gc.thicknessint
...
let draw_points (g: gctx) (pl: position list) : unit =
  List.iter (fun x -> draw_point g x) pl
