...
(** TODO: The first part of task 5 requires you to implement the following
    ...
    Created two records that are updated using the fields of value_controller
    *)

let make_controller (v: 'a) : 'a value_controller =
  let listeners = {contents = []} in 
  let value = {contents = v} in
  {add_change_listener = (fun f -> listeners.contents <- f::listeners.contents);
  get_value = (fun () -> value.contents);
  change_value = (fun c -> value.contents <- c; 
  List.iter (fun a -> a value.contents) listeners.contents)}
...
let checkbox (init: bool) (s: string) : widget * bool value_controller =
  let check = make_controller init in
  let w = {repaint = (fun (g: Gctx.gctx) -> 
  Gctx.draw_string g (10, 0) s;
    if check.get_value() then (Gctx.fill_rect g (0,5) (10, 10)) else
    Gctx.draw_rect g (0,5) (10, 10));
  handle = (fun (g: Gctx.gctx)(e: Gctx.event) ->
  (*If detecting an event, change check's value*)
    begin match Gctx.event_type e with
    | Gctx.MouseDown -> if (event_within g e (60, 20)) then 
    update_value check (fun (a: bool) -> not a);
    | _ -> () 
    end);
  size = fun () -> (60,20)
  } in
...
let slider (init: int) (s: string) : widget * int value_controller =
  let slide = make_controller init in
  (*Creating a height and width for the slider*)
  let max_val = 100 in
  let min_val = 15 in
  (*Creating a value controller*)
  let w = {repaint = (fun (g: Gctx.gctx) ->
  (*Repainting a filled rectangle based off of the value of the slider*) 
  let x = slide.get_value() in
    Gctx.fill_rect g (0, min_val) (x, max_val));
  handle = (fun (g: Gctx.gctx)(e: Gctx.event) ->
  (*Changing the slider value based on the mouse position when it drags*)
    begin match Gctx.event_type e with
    | Gctx.MouseUp -> let (x,y) = Gctx.event_pos e g in slide.change_value(x);
    | Gctx.MouseDrag -> let (x,y) = Gctx.event_pos e g in slide.change_value(x);
    | _ -> () 
    end);
  (*size of the widget*)
  size = fun () -> (max_val, max_val)
  } in
  (w, slide)
